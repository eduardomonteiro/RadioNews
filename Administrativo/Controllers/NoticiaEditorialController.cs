...
{
    [CustomAuthorize(Roles = "Administrador,Editorial Noticias")]
    public class NoticiaEditorialController : BaseController
    {
        protected const string pathFoto1361x750 = "~/Conteudo/noticias/{0}/1361x750/";
        ...
        public ActionResult Index(string editorial, int? page, string search = "", string sortField = "id", string order = "DESC", int EditoriaId = 0)
        {
            ...
            var editoriais = db.Editoriais.Where(a => !a.excluido && (!a.especial || a.chave == "bh-fiscaliza")).ToList();

            //var editoriais = db.Editoriais.Where(a => !a.excluido && (!a.especial || a.chave == "bh-fiscaliza" || a.chave == "carnaval-2017")).ToList();
        ...
        public IPagedList<Noticias> GetListItens(string editorial, int? page, string searchParam = "", string sortField = "", string order = "", int EditoriaId = 0)
        {
            ...
            var list = db.Noticias
                ...
                a.Editoriais.Any(x => !x.esportes && (x.especial == false || x.chave == "bh-fiscaliza")));
            ...
            if (!string.IsNullOrEmpty(searchParam))
            {
                ...
                pagedList = pagedList.Where(n => n.chamada.Contains(searchParam));
            ...
            switch (sortField)
            {
                ...
                case "editoria":
                    if (order == "ASC")
                    {
                        pagedList = pagedList.OrderBy(n => n.Editoriais.FirstOrDefault().nome);
                        ViewBag.SortClasseditoria = "sorting_asc";

                    }
                    else
                    {
                        pagedList = pagedList.OrderByDescending(n => n.Editoriais.FirstOrDefault().nome);
                        ViewBag.SortClasseditoria = "sorting_desc";
                    }
                    break;
                case "destaqueHome":
                    if (order == "ASC")
                    {
                        pagedList = pagedList.OrderBy(n => ((TipoDestaque)n.TipoDestaque));
                        ViewBag.SortClassdestaqueHome = "sorting_asc";

                    }
                    else
                    {
                        pagedList = pagedList.OrderByDescending(n => ((TipoDestaque)n.TipoDestaque));
                ...
                case "plantao":
                    if (order == "ASC")
                    {
                        pagedList = pagedList.OrderBy(n => n.plantao);
                        ViewBag.SortClassplantao = "sorting_asc";

                    }
                    else
                    {
                        pagedList = pagedList.OrderByDescending(n => n.plantao);
                        ViewBag.SortClassplantao = "sorting_desc";
                    }
                    break;
                case "ativo":
                    if (order == "ASC")
        ...
        public ActionResult Create(int EditoriaId = 0)
        {
            ...
            ViewBag.EditoriaId = new SelectList(editoriais, "id", "nome", selected);
            ViewBag.CategoriaId = new SelectList(db.Categorias.Where(a => !a.Excluido && a.EditoriaId == selected).ToList(), "Id", "Titulo");
        ...
        [HttpPost]
        public ActionResult Create(Noticias noticias, HttpPostedFileBase audio, HttpPostedFileBase foto, int[] CategoriaId, int[] EditoriaId, string ListaTags = "", string fotoExistente = "", string audioExistente = "", bool enviarPush = false, string titlePush = "", string messagePush = "")
        {
            #region Validacoes
            ...
            if (foto == null && (noticias.TipoDestaque.Value == (int)TipoDestaque.Normal1 || noticias.TipoDestaque.Value == (int)TipoDestaque.Normal2 || noticias.TipoDestaque.Value == (int)TipoDestaque.Normal3 || noticias.TipoDestaque.Value == (int)TipoDestaque.Urgente1130 || noticias.TipoDestaque.Value == (int)TipoDestaque.Urgente360 || noticias.TipoDestaque.Value == (int)TipoDestaque.Editoria))
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                */
                ...
                if (db.SaveChanges() > 0)
                {
                    ...
                    do
                    {
                        noticias.url = (
                            ...
                            noticias.TituloCapa.GenerateSlug() : noticias.titulo.GenerateSlug()
                ...
                if (enviarPush)
                {
                    using (var notificacaoController = new NotificacoesPushController())
                    {
                        notificacaoController.GerarNotificacoes(noticias, messagePush, titlePush);
                    }
                }
            ...
            if (EditoriaId.Any())
            {
                ViewBag.EditoriaEspecialNome = editoriais.FirstOrDefault(x =>EditoriaId.Contains(x.id)).nome;
            }

            ViewBag.EditoriaId = new SelectList(editoriais, "id", "nome", selected);

            ViewBag.CategoriaId = new SelectList(db.Categorias.Where(a => !a.Excluido).ToList(), "Id", "Titulo");
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            ViewBag.TiposDestaques = (from Enum item in Enum.GetValues(typeof(TipoDestaque))
                                      select new SelectListItem
                                      {
                                          Value = ((int)(object)item).ToString(),
                                          Text = item.GetDescription(),
                                          Selected = ((int)(object)item == (int)TipoDestaque.Editoria) ? (noticias.destaqueEditoria) : false
                                      }).ToList();
            ...
            var editorial = editoriais.FirstOrDefault(a => !a.excluido && a.Categorias.Any(x => x.Id == catId));

            int firstId = editorial == null ? editoriais.First().id : editorial.id;
        ...
        {
            var fileOriginal = Server.MapPath("~/Conteudo/noticias/outros/");
            var ext = Path.GetExtension(Request.Files[0]?.FileName);
            var extensoesPermitidas = new List<string> {".jpg", ".jpeg", ".png", ".bmp"};
            if (!extensoesPermitidas.Contains(ext?.ToLower())) return Json("");
            var filename = Utils.SaveFileBase(fileOriginal, Request.Files[0]);
            return !string.IsNullOrEmpty(filename) ? Json("/Admin/Conteudo/noticias/outros/" + filename) : Json("");
        }
