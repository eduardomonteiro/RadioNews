...
$.fn.rangeCalendar = function(options) {
	...
	this.each(function(i, el) {
		...
		obj.didResizeBar = function() {
			...
			obj._dispatchEvent(obj.changeRangeCallback,obj.range("day"),obj);
		},
		obj.didSelectMonth = function(e) {
			...
			obj._dispatchEvent(obj.changeRangeCallback,obj.range("month"),obj);	
			
		},
        obj.dayTransactionEffect = function (startDate)
        {
            var date = moment(startDate);
            var fullYear = date.format("YYYY");
            var monthNumber = date.format("MM");
		...
		obj._getCalendarHTML = function(startDate,endDate) {
			...
			for (var index = 1; (date.isBefore(endDate) || date.isSame(endDate)) ; index++){
				...
				cell += '<div class="month">'+month+'</div>';
				cell += '</div>';
				cell += '</div>';
		...
		obj._generateView = function () {
			...
			obj.calendarObj = $('<div class="wrapper"><div id="arrow-right" class="fa fa-angle-right"></div><div id="arrow-left" class="fa fa-angle-left"></div><div class="calendar"></div></div>');
			$el.append( obj.calendarObj );
			obj.calendarObj = obj.calendarObj.find(".calendar");

			//var month = obj.startDate.getMonth();

			//var year = obj.startDate.getFullYear();

			//obj.startDate = new Date(year, month);
